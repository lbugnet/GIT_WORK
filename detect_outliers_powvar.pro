...
;IN
 OUTPUT_RESIZE=OUTPUT_RESIZE, YY=YY, XX=XX, RES=RES, THRESHOLD=THRESHOLD, FLAG=FLAG, $
  ...
  HELP=HELP, PATH_OUTPUT=PATH_OUTPUT, PATH_DATA=PATH_DATA, PATH_TABLE=PATH_TABLE, GLOBAL=GLOBAL
...
if n_elements(PATH_DATA) eq 0 then PATH_DATA='/Volumes/TEMP/'
 ...
 ;--------------------------------------------------------------
  ...
  pp=plot(10^xx, 10^yy,dim=[800,400],xlog=1, ylog=1,xr=size, font_size=13, xtitle=PARAM+'  $(\mu Hz)$', font_name='Times', ytitle='$POWVAR (ppm^2/\mu Hz)$',symbol=".", sym_size=2, SYM_FILLED=1, color="black",linestyle="none", transparency=50);'title="ALL KEPLER RED GIANTS WITH M AND NUMAX KNOWN")
  pp5=plot(10^xx, 10^(slope_fit+threshold),xlog=1, ylog=1, color="lime green",linestyle=1 ,/overplot, transparency=50 )
  pp5.thick=2
  pp5=plot(10^xx, 10^(slope_fit), xlog=1, ylog=1,color="orange",linestyle=1 ,/overplot, transparency=50 )
  pp5.thick=2
  pp4=plot(10^xx, 10^(slope_fit-threshold),xlog=1, ylog=1, color="dark magenta",linestyle=1 ,/overplot, transparency=50 )
  pp4.thick=2
